...
def write_equality(var_value_list, f):
    write_constraint(var_value_list, False, f)
    f.write("\n")
    write_constraint(var_value_list, True, f)

def write_constraint(var_value_list, flipsign, f):
    c = -1 if flipsign else 1
    for var, value in var_value_list:
        f.write(f"  lp.set_a({var}, C, {int(c*value)});\n")
    f.write("   lp.set_b(C++, 0);\n")

def count_cells_by_trails(valid_trails, cell, f):
    ...
    write_equality(trails_containing_cell, f)
